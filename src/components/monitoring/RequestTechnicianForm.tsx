...
interface RequestTechnicianFormProps {
  systemId: string
  systemName: string   // passed from page for nice heading
}

export default function RequestTechnicianForm({ systemId, systemName }: RequestTechnicianFormProps) {
  ...
  const [state, formAction, isPending] = useActionState<FormState, FormData>(
    async (_prevState, formData) => {
      try {
        ...
        return {
          error: err.message || "Failed to submit service request. Please try again.",
        }
      }
    },
    { success: false }
  ...
  useEffect(() => {
    if (state.success && state.requestId) {
      // Redirect to a confirmation or back to system detail
      // You can later create /dashboard/service-request/[id] if needed
  ...
  return (
    <form action={formAction} className="max-w-2xl space-y-6 mx-auto px-4 py-8">

      <h2 className="text-2xl font-semibold text-center">
        Request Technician for <span className="text-gray-700">{systemName}</span>
      </h2>
      ...
      <div>
        ...
        >
          ...
          <option value="LOW_GENERATION">Lower than expected generation</option>
          <option value="BATTERY_ISSUE">Battery not charging / draining fast</option>
          <option value="INVERTER_FAULT">Inverter error / beeping</option>
          <option value="PANEL_DAMAGE">Panel damage / dirt</option>
          <option value="WIRING">Wiring / connection problem</option>
